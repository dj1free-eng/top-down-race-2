...
const gridSpawns = Array.isArray(t01?.grid?.slots)
  ? t01.grid.slots
...
// Líneas blancas desactivadas temporalmente.
  ...
  state.targetAngle = Math.atan2(state.stickY, state.stickX) - (Math.PI / 2);
    ...
    if (typeof trackKey === 'string' && trackKey.trim()) {
      try {
        ...
        console.warn('[RaceScene] track no encontrado:', trackKey, e);
      }
    }
    ...
    try {
      ...
      console.error('[RaceScene] no se pudo cargar track01', e);
      return null;
    }
  }
