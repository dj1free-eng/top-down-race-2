...
renderUI() {
  ...
  // IMPORTANTÍSIMO: function (no const) para evitar "M before initialization"
  function drawPill(gg, w, selected) {
    gg.clear();
  ...
  function renderCarStats() {
    const carId = self.selectedCarId || 'stock';
  ...
  let totalW = 0;
  for (let i = 0; i < pills.length; i++) totalW += pills[i].w;
...
const topY = Math.max(cardsTopMin, Math.floor(height * (isPortrait ? 0.50 : 0.56)));
  ...
  function makeCard(x, y, t) {
    ...
    const bg = self.add.rectangle(0, 0, cardW, cardH, 0x0b1020, 0.35)
      .setStrokeStyle(1, 0xb7c0ff, 0.25)
      .setOrigin(0);

    const header = self.add.rectangle(0, 0, cardW, 44, 0x0b1020, 0.55).setOrigin(0);

    const tag = self.add.text(cardW - 16, 13, t.tag, {
      fontFamily: 'system-ui, -apple-system, Segoe UI, Roboto, Arial',
      fontSize: '12px',
      color: '#b7c0ff',
      fontStyle: 'bold'
    ...
    const title = self.add.text(16, 12, t.title, {
      fontFamily: 'system-ui, -apple-system, Segoe UI, Roboto, Arial',
      fontSize: '14px',
      color: '#ffffff',
      fontStyle: 'bold',
    ...
    const desc = self.add.text(16, 60, t.desc, {
      fontFamily: 'system-ui, -apple-system, Segoe UI, Roboto, Arial',
      fontSize: '13px',
      color: '#b7c0ff',
      lineSpacing: 5,
    ...
    const cta = self.add.text(16, cardH - 28, 'Seleccionar →', {
      fontFamily: 'system-ui, -apple-system, Segoe UI, Roboto, Arial',
      fontSize: '13px',
      color: '#2bff88',
      fontStyle: 'bold'
    ...
    bg.on('pointerdown', () => {
      self.registry.set('selectedTrack', t.key);

      toast.setText(`Seleccionado: ${t.title}`);
      self.tweens.killTweensOf(toast);
      toast.setAlpha(0);
      self.tweens.add({ targets: toast, alpha: 1, duration: 120, yoyo: true, hold: 450 });

      self.time.delayedCall(250, () => {
        self.scene.start('race', { carId: self.selectedCarId, trackKey: t.key });
      });
    });
