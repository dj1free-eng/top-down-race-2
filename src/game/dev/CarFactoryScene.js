...
ensureCarSkinTexture(spec) {
  ...
  this.cameras.main.setBackgroundColor(0x0a0e27);
    ...
    if (!this._errOverlay) {
      this._errOverlay = this.add.text(10, 10, "", {
      ...
      const pushErr = (msg) => {
        try {
          const cur = this._errOverlay.text || "";
          this._errOverlay.setText((cur ? cur + "\n\n" : "") + String(msg).slice(0, 1200));
        } catch (_) {}
      };

      window.addEventListener("error", (e) => {
        pushErr("window.error: " + (e.message||"?") + " @ " + (e.filename||"?") + ":" + (e.lineno||"?") + ":" + (e.colno||"?"));
      });
      window.addEventListener("unhandledrejection", (e) => {
        pushErr("unhandledrejection: " + String(e.reason));
      });

      // Si hay un fallo antes de pintar UI, al menos ver00E1s esto
      pushErr("(debug) CarFactoryScene arranc00F3");
    }
